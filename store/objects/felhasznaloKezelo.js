...
var FelhasznaloKezelo = function(Adatkoto) {
    ...
    var frissites = function() {
        if(!szerkesztes) {
            if(!bejelentkezve) {
                $(adatkoto).html(
                    "<table><tr><td><span class='link login'>Bejelentkezés</span></td><td><span class='link signup'>Regisztráció</span></td></tr>" +
                        "</table>"
                );
            }
            else {
        ...
        $(adatkoto + " .signup").click(function() {
            var event = new Event('signup');
            document.dispatchEvent(event);
        });
