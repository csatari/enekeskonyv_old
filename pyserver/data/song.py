...
class Song(BaseClass):
	...
	def add(self,title,text,notes,lang,otherlang,labels,comments,songid,sessionid):
		...
		songid = self.graph.cypher.execute_one("""MATCH (l:Language),(u:User)
												...
												RETURN ID(s) as songid""" % (lang,user.userid,otherlang,songid,title,text,notes,comments))
		self.add_labels_to_song(labels,songid)
		return songid
	...
	def get_creator_of_song(self,songid):
		userid = self.graph.cypher.execute_one("MATCH (s:Song)-[:CREATED_BY]->(u:User) where ID(s) = %s return ID(u) as userid" % songid)
		if userid == None:
			return 0
		else:
			return userid
	...
	def to_json_one_pure(self,node):
		...
		return json.dumps({"id": node.songid, "version": version_number, "creator": node.userid, "title": self.get(node.s,"title"),
						   "song": self.get(node.s,"song"), "sheet_music": self.get(node.s,"sheet"), "language": node.langid, 
						   "other_languages": other_lang_id,"labels": self.get_all_labels_as_string(node.songid), 
						   "comment": self.get(node.s,"comment")})
